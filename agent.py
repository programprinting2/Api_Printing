...
def api_file_info():
    ...
    try:
        if ext == 'pdf':
            ...
            stat = os.stat(filepath)
            return jsonify({'status':'success','file_size_bytes': stat.st_size, 'file_size_mb': round(stat.st_size/(1024*1024),2)})
    except Exception as e:
...
def ui_merge():
    try:
        ...
        if not file1 or not file2 or not output:
            return jsonify({"status": "error", "message": "Missing file or output filename"}), 400
...
def ui_read_pdf_info():
    try:
        ...
        if not file_path:
            ...
            print(f"DEBUG: os.path.isfile(file_path) = {os.path.isfile(file_path)}", flush=True)
            sys.stdout.flush()
